...
import { DollarSign, ArrowUp, ArrowDown, PiggyBank, TrendingUp, RefreshCw, Clock, Utensils, Home, Car, ShoppingCart, Gift, Activity, CreditCard, Zap, Info, ChevronDown, ChevronUp, PieChart } from 'lucide-react';
...
const Layout = ({ onLogout, user }) => {
    ...
    const [_showAllTransactions, _setShowAllTransactions] = useState(false);
    const [lastUpdated, setLastUpdated] = useState(new Date());
    const [sidebarCollapsed, setSidebarCollapsed] = useState(false);

    // to fetch transaction over sever side
    ...
    const _displayedTransactions = _showAllTransactions
        ? transactions
                        ...
                        <div className={styles.cards.base}>
                            <div className={styles.cards.header}>
                                <h3 className={styles.cards.title}>
                                    ...
                                    Financial Overview
                                    <span className="text-sm text-grey-500 font-normal">
                                        {_timeFrameLabel}
                                    </span>
                                </h3>
                            </div>
                            <Outlet context={_outletContext} />
                        </div>
                    </div>
                    {/* right side */}
                    <div className={styles.grid.rightColumn}>
                        <div className={styles.cards.base}>
                            <div className={styles.transactions.cardHeader}>
                                ...
                                >
                                    <RefreshCw className={styles.transactions.refreshIcon(_loading)} />
                                </button>
                            </div>
                            ...
                            <div className={styles.transactions.listContainer}>
                                {_displayedTransactions.map((transaction) => {
                                ...
                                {transactions.length === 0 ? (
                                    ...
                                    <div className={styles.transactions.viewAllContainer}>
                                        <button
                                            onClick={() => _setShowAllTransactions(!_showAllTransactions)}
                            ...
                            <div className={styles.cards.base}>
                                ...
                                <div className={styles.categories.list}>
                                    {_topCategories.map(([category, amount]) => (
